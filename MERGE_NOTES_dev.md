...
| Permutation test | Exact enumeration for ≤ 5000 splits (typical rMATS designs), observed ΔΨ from rMATS | For 2v2/3v3 designs p-values move from Monte-Carlo estimates with K=500 to exact k/6 or k/20; `pct_p05` unchanged in expectation for 3v3 (min p = 0.05 counts as < 0.05? no: 0.05 is not < 0.05 → `pct_p05` can **drop to 0** for 3v3 designs where the old Monte-Carlo estimate sometimes fell just below 0.05) | fewer "significant" events in small designs, honest reporting |
