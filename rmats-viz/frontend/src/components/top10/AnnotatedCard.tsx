 ...
 *   conservation – Link to Ensembl Compara cross-species alignment
 */
  ...
  return (
    <div className="space-y-2 text-sm">
      ...
      <div className="text-xs text-muted-foreground bg-muted/40 rounded px-3 py-2 space-y-0.5">
        <div>
          <span className="font-medium text-foreground">Chr :</span>{" "}
          {ev.chr ?? "?"} {ev.strand ? `(brin ${ev.strand})` : ""}
        </div>
        <div>
          <span className="font-medium text-foreground">Exon :</span>{" "}
          {formatCoord(ev.exon_start)} – {formatCoord(ev.exon_end)}
        </div>
        <div>
          <span className="font-medium text-foreground">Upstream :</span>{" "}
...
function PanelAppView({ annotation, isLoading }: { annotation?: GeneAnnotation; isLoading: boolean }) {
  ...
  return (
    <div className="space-y-1.5">
      ...
      <a
        href={`https://panelapp.agha.umccr.org/panels/entities/${encodeURIComponent(annotation?.symbol ?? "")}/`}
        target="_blank"
        rel="noopener noreferrer"
  ...
  return (
    <div className="space-y-2 text-xs">
      ...
      <div className="bg-muted/40 rounded px-2.5 py-2 space-y-1">
        <p className="font-semibold text-foreground text-[10px] uppercase tracking-wide mb-1">Comptages (3 premiers échantillons)</p>
        {ev.ijc_sample_1 && (
          <p><span className="text-muted-foreground">IJC G1:</span>{" "}
            {ev.ijc_sample_1.split(",").slice(0, 3).join(", ")}{ev.ijc_sample_1.split(",").length > 3 ? "…" : ""}
          </p>
        )}
        {ev.sjc_sample_1 && (
          <p><span className="text-muted-foreground">SJC G1:</span>{" "}
            {ev.sjc_sample_1.split(",").slice(0, 3).join(", ")}{ev.sjc_sample_1.split(",").length > 3 ? "…" : ""}
          </p>
        )}
        {ev.ijc_sample_2 && (
          <p><span className="text-muted-foreground">IJC G2:</span>{" "}
            {ev.ijc_sample_2.split(",").slice(0, 3).join(", ")}{ev.ijc_sample_2.split(",").length > 3 ? "…" : ""}
          </p>
        )}
        {ev.sjc_sample_2 && (
          <p><span className="text-muted-foreground">SJC G2:</span>{" "}
            {ev.sjc_sample_2.split(",").slice(0, 3).join(", ")}{ev.sjc_sample_2.split(",").length > 3 ? "…" : ""}
          </p>
        )}
      </div>
    </div>
...
function ConservationView({ ev }: { ev: SplicingEvent }) {
  const ensgId = ev.gene_id;
  const comparaLink = ensgId
    ? `https://www.ensembl.org/Homo_sapiens/Gene/Compara_Tree?g=${ensgId}`
    : null;
  const alignLink = ensgId
    ? `https://www.ensembl.org/Homo_sapiens/Gene/Compara_Alignments?g=${ensgId}`
    : null;

  return (
    <div className="space-y-3 text-sm">
      <p className="text-xs text-muted-foreground leading-relaxed">
        La conservation inter-espèce est consultable directement sur Ensembl Compara.
      </p>
      <div className="space-y-2">
        {comparaLink && (
          <a
        ...
        {alignLink && (
          ...
          >
            ...
            Alignements génomiques
          </a>
        )}
        {!ensgId && (
          <p className="text-xs text-muted-foreground italic">
            Identifiant Ensembl non disponible pour ce gène.
          </p>
        )}
      </div>
    </div>
...
// ---------------------------------------------------------------------------
// GeneSymbol with tooltip (protein function on hover)
// ---------------------------------------------------------------------------
...
export function AnnotatedCard({ event: ev, mode, ensemblIdHint }: AnnotatedCardProps) {
  const symbol = ev.gene_symbol ?? ev.gene_id ?? "";

  // Fetch annotation only when we have a gene symbol and mode requires it
  ...
  return (
    <div className="flex flex-col border border-border rounded-xl bg-card shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="p-4 flex-1">
        ...
        {mode === "conservation" && <ConservationView ev={ev} />}
      </div>
    </div>
  );
}
